...
public:
    Trapezoid() { pts.resize(4); for (int i=0;i<4;++i) pts[i] = std::make_unique<point_t>(); }
    Trapezoid(const std::initializer_list<point_t>& l) {
        pts.resize(4);
        int i=0;
    ...
    bool equals(const Figure<T>& other) const override {
        auto p = dynamic_cast<const Trapezoid<T>*>(&other);
        if (!p) return false;
        for (size_t i=0;i<4;++i) {
